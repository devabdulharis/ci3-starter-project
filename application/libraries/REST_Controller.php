...
abstract class REST_Controller extends \CI_Controller
{
    ...
    {
        $key = $this->_args[$this->config->item('rest_key_name')] ?? null;
        if ($key) {
            $this->db->where('key', $key);
            $row = $this->db->get('keys')->row();
            if ($row) {
                $client_ip = $this->input->ip_address();

                if (!empty($row->ip_addresses)) {
                    $allowed_ips = array_map('trim', explode(',', $row->ip_addresses));

                    if (!in_array($client_ip, $allowed_ips)) {
                        $this->response([
                            'status' => FALSE,
                            'message' => 'IP '.$client_ip.' not allowed for this API key.'
                        ], self::HTTP_UNAUTHORIZED);
                        return FALSE;
                    }
                }
